...
function Game(params) {
  ...
  this.roleCardSamplesKeys = [r_fsci, r_invst, r_invst, r_mrd, r_invst, r_invst, r_accmp, r_invst, r_invst, r_wtnss, r_invst, r_invst];
...
Game.prototype.assignRoles = function() {
  ...
  for (var i = playerCount - 1; i > -1 ; i--) {
    var player = this.players[playerKeys[i]];
    if (player.name.endsWith('_fsci'))
    {
      indicesPlayers.splice(i,1);
      indicesRoles.splice(1,1);
      player.role = r_fsci;
      //forcedAssignments++;
    }
    if (player.name.endsWith('_mrd'))
    {
      indicesPlayers.splice(i,1);
      indicesRoles.splice(0,1);
      player.role = r_mrd;
      //forcedAssignments++;
    }
  }
  ...
  var playerKeys = Object.keys(this.players);
  //if(forcedAssignments < indicesPlayers.length)
